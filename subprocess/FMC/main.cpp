...
int main(){
	...
	char* mySharedMemoryFMCA = WideCharToString(mySharedMemoryFMCW);
	
	// 打开共享内存
	HANDLE hMapFileFMC = OpenFileMapping(
		FILE_MAP_ALL_ACCESS,    // 可读可写
		FALSE,                  // 不从继承句柄中映射
		mySharedMemoryFMCW // 共享内存名称
		);
	...
	// 映射共享内存到进程地址空间
	SharedDataFMC* pDataFMC0 = (SharedDataFMC*)MapViewOfFile(
		hMapFileFMC,               // 共享内存句柄
		FILE_MAP_ALL_ACCESS,    // 可读可写
		0,                      // 高位文件偏移
		0,                      // 低位文件偏移
...
//	WCHAR eventFMCW[] =  L"EventDataAvailable";
//	char* eventFMCA  = WideCharToString(eventFMCW);
//	
//	// 打开事件
//	HANDLE hEventFMC = OpenEvent(
//		EVENT_ALL_ACCESS,
//		FALSE, 
//		eventFMCA
//		);
...
//	}
//	
//	hEventFMC0 = hEventFMC;
	
	pDataFMC = pDataFMC0;
	
	
	egeFMCHeight = pDataFMC ->egeFMCHeight;
	egeFMCWidth = pDataFMC ->egeFMCWidth;
	...
	for ( ; is_run(); delay_fps(60) )
	{
		...
		draw_FMC(windowsWidth * 1.0 / 2,windowsHeight * 1.0 / 1.4,windowsHeight*0.45);
		
		MainPanelLight = pDataFMC ->MainPanelLight;
		
		ceshi ++;
		pDataFMC -> ceshi = ceshi;
		printf("%d\n\n\n\n",pDataFMC->ceshi);
	}
	...
	UnmapViewOfFile(pDataFMC0);
	free(mySharedMemoryFMCA);
//	CloseHandle(hEventFMC);
//	CloseHandle(hEventFMC0);
	return 0;
}
	...
	if (FMC_mouse_real_x >= center_x - length / 2 && FMC_mouse_real_x <= center_x + length / 2 && FMC_mouse_real_y >= center_y - width / 2 && FMC_mouse_real_y <= center_y + width / 2) {
		if (GetAsyncKeyState(0x02) & 0x0001) 
		{
			...
			if (FMC_mode == 8)
			{
				...
				{
					strcpy(pDataFMC-> FMCFileName,getinformation_9(1));
				}
				else if (fabs(center_x - FMC_origin_center_x_up + single * 4.5 * 13.33333333 / 67 * 42) < 0.0001 && fabs(center_y - FMC_origin_center_y_up + single * 4.5 * 13.33333333 / 67 * 8) < 0.0001)
				{
					strcpy(pDataFMC-> FMCFileName,getinformation_9(3));
				}
				else if (fabs(center_x - FMC_origin_center_x_up + single * 4.5 * 13.33333333 / 67 * 42) < 0.0001 && fabs(center_y - FMC_origin_center_y_up + single * 4.5 * 13.33333333 / 67 * 1) < 0.0001)
				{
					strcpy(pDataFMC-> FMCFileName,getinformation_9(5));
				}
				else if (fabs(center_x - FMC_origin_center_x_up + single * 4.5 * 13.33333333 / 67 * 42) < 0.0001 && fabs(center_y - FMC_origin_center_y_up - single * 4.5 * 13.33333333 / 67 * 6) < 0.0001)
				{
					strcpy(pDataFMC-> FMCFileName,getinformation_9(7));
				}
				else if (fabs(center_x - FMC_origin_center_x_up - single * 4.5 * 13.33333333 / 67 * 42) < 0.0001 && fabs(center_y - FMC_origin_center_y_up + single * 4.5 * 13.33333333 / 67 * 15) < 0.0001)
				{
					strcpy(pDataFMC-> FMCFileName,getinformation_9(2));
				}
				else if (fabs(center_x - FMC_origin_center_x_up - single * 4.5 * 13.33333333 / 67 * 42) < 0.0001 && fabs(center_y - FMC_origin_center_y_up + single * 4.5 * 13.33333333 / 67 * 8) < 0.0001)
				{
					strcpy(pDataFMC-> FMCFileName,getinformation_9(4));
				}
				else if (fabs(center_x - FMC_origin_center_x_up - single * 4.5 * 13.33333333 / 67 * 42) < 0.0001 && fabs(center_y - FMC_origin_center_y_up + single * 4.5 * 13.33333333 / 67 * 1) < 0.0001)
				{
					strcpy(pDataFMC-> FMCFileName,getinformation_9(6));
				}
				else if (fabs(center_x - FMC_origin_center_x_up - single * 4.5 * 13.33333333 / 67 * 42) < 0.0001 && fabs(center_y - FMC_origin_center_y_up - single * 4.5 * 13.33333333 / 67 * 6) < 0.0001)
				{
					strcpy(pDataFMC-> FMCFileName,getinformation_9(8));
				}
			}
			/*draw_RouteTable_Side_Button(center_x - single * 42, center_y - single * 15, side / 13.33333333);
			draw_RouteTable_Side_Button(center_x - single * 42, center_y - single * 8, side / 13.33333333);
...
//	printf("\n");
	...
	for (int i = 0; i < 12; i++) {
		if (strcmp(FMC_lastroutepoints[i],FMC_routepoints[i]) != 0) {
			...
			for (int i = 0; i < 12; ++i)
				for (int j = 0; j < 7; ++j)
					pDataFMC->FMC_routepoints[i][j] = FMC_routepoints[i][j];
