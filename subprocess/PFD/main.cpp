...
int main() {
	...
	char* mySharedMemoryPFDA = WideCharToString(mySharedMemoryPFDW);

	// 打开共享内存
	HANDLE hMapFilePFD = OpenFileMapping(
		FILE_MAP_ALL_ACCESS,    // 可读可写
		FALSE,                  // 不从继承句柄中映射
		mySharedMemoryPFDW // 共享内存名称
	);
	...
	// 映射共享内存到进程地址空间
	SharedDataPFD* pDataPFD0 = (SharedDataPFD*)MapViewOfFile(
		hMapFilePFD,               // 共享内存句柄
		FILE_MAP_ALL_ACCESS,    // 可读可写
		0,                      // 高位文件偏移
		0,                      // 低位文件偏移
	...
	printf("hah\n");
	printf("pDataPFD0 airspeed : %lf\n\n\n\n", pDataPFD0->airSpeed);

	pDataPFD = pDataPFD0;

	printf("pDataPFD airspeed : %lf\n\n\n\n", pDataPFD->airSpeed);

	egeHeight = pDataPFD->egeHeight;
	egeWidth = pDataPFD->egeWidth;
	...
	//	style &= ~(WS_CAPTION | mmWS_THICKFRAME | WS_SYSMENU | WS_MINIMIZEBOX | WS_MAXIMIZEBOX);
	//	style &= ~(WS_CAPTION);
	SetWindowLong(hwnd, GWL_STYLE, style);

	// 计算并设置新窗口大小以适应无标题栏的情况
	RECT rect;
	GetClientRect(hwnd, &rect); // 获取客户区的尺寸
	//AdjustWindowRect(&rect, style, FALSE); // 调整为包括非客户区的窗口尺寸
	SetWindowPos(hwnd, NULL, 0, 0, rect.right - rect.left, rect.bottom - rect.top, SWP_NOZORDER | SWP_NOMOVE | SWP_FRAMECHANGED);
	...
	for (; is_run(); delay_fps(60))
	{
		...
		draw_PFD(windowsWidth * 1.0 / 2.25, windowsHeight * 1.0 / 2.05, windowsHeight * 0.96);

		//		egeHeight = pDataPFD ->egeHeight;
		//		egeWidth = pDataPFD ->egeWidth;

		OUTBDangle = pDataPFD->OUTBDangle;

		pitchAngle = pDataPFD->pitchAngle;
		rollAngle = pDataPFD->rollAngle;
		sideSlipAngle = pDataPFD->sideSlipAngle;
		headingIndication = pDataPFD->headingIndication;
		glideIndication = pDataPFD->glideIndication;

		airSpeed = pDataPFD->airSpeed;
		airSpeed_acceleration = pDataPFD->airSpeed_acceleration;
		airSpeed_mach = pDataPFD->airSpeed_mach;
		airSpeed_instruction = pDataPFD->airSpeed_instruction;

		MTRS = pDataPFD->MTRS;
		altitude = pDataPFD->altitude;
		indicated_number = pDataPFD->indicated_number;
		altitude2 = pDataPFD->altitude2;
		rotationangle = pDataPFD->rotationangle;
		//pointrotationangle = pDataPFD ->pointrotationangle;
		course = pDataPFD->course;

		verticalRate = pDataPFD->verticalRate;

		attackAngle = pDataPFD->attackAngle;
...
void draw_PFD(double PFD_x, double PFD_y, double PFD_side) {
	...
	//灯光遮盖效果
	setfillcolor(EGEARGB(255 - (int)pDataPFD->OUTBDangle, 0x00, 0x00, 0x00));
	...
	for (double i = -90; i <= 90; i += 2.5) {
		if (i == 0) {
			...
			if (sqrt((AI_gear_x - AI_x) * (AI_gear_x - AI_x) + (AI_gear_y - AI_y) * (AI_gear_y - AI_y)) <= 1.5 * AI_side) {
				char tmp[3];
				sprintf_s(tmp, "%d", abs((int)i));
				outtextxy(AI_gear_x, AI_gear_y, tmp);
			}
...
void draw_PFD_AI_alt() {
	...
	if (altitude2 <= 2500) {
		...
		char num[10];
		sprintf_s(num, "%.0f", altitude2);
		setcolor(WHITE);
		setfont(0.2 * AI_side, 0.1 * AI_side, "黑体");
		outtextxy(AI_x, AI_y + AI_side * 86 / 96, num);
	}
...
void draw_PFD_alt_indicatedNum(double side, double center_x, double center_y) {
	...
	if (indicated_number < 10000) {
		...
		outtextxy(x - 0.015 * side,  y - 0.04 * side, last);
		...
		number_height = side * 0.055 * 1.3;
		number_width = side * 2.5 / 96.0 * 0.85 * 1.3;
		outtextxy(x - 0.02 * side,  y - 0.08 * side, last);
...
void draw_PFD_RCDI(double RCDI_x, double RCDI_y, double RCDI_side) {
	...
	setfont(Ulength / 9 * 25, Ulength / 9 * 10, "Calibri");

	outtextxy(RCDI_x - 2.8 * Ulength, RCDI_y - 22.4 * Ulength, "6");
	outtextxy(RCDI_x - 2.8 * Ulength, RCDI_y + 19.5 * Ulength, "6");
	outtextxy(RCDI_x - 2.8 * Ulength, RCDI_y - 18.7 * Ulength, "2");
	outtextxy(RCDI_x - 2.8 * Ulength, RCDI_y + 15.5 * Ulength, "2");
	outtextxy(RCDI_x - 2.8 * Ulength, RCDI_y - 10.5 * Ulength, "1");
	outtextxy(RCDI_x - 2.8 * Ulength, RCDI_y + 7.5 * Ulength, "1");


	setfont(Ulength / 9 * 35, Ulength / 9 * 10, "Calibri");
	if (verticalRate >= 400 && verticalRate <= 9999)
	{
		char num[64];
		sprintf_s(num, "%d", (int)verticalRate / 50 * 50);
		outtextxy(RCDI_x - 1 * Ulength, RCDI_y - 26.5 * Ulength, num);

	}
	else if (verticalRate > 9999)
	{
		outtextxy(RCDI_x - 1 * Ulength, RCDI_y - 26.5 * Ulength, "9999");
	}
	else if (verticalRate <= -400 && verticalRate >= -9999)
	{
		char num[64];
		sprintf_s(num, "%d", (int)-verticalRate / 50 * 50);
		outtextxy(RCDI_x - 1 * Ulength, RCDI_y + 24 * Ulength, num);
	}
	else if (verticalRate < -9999)
	{
		outtextxy(RCDI_x - 1 * Ulength, RCDI_y + 24 * Ulength, "9999");
	}
...
void draw_course(double compass_x, double compass_y, double side) {
	...
	outtextxy(compass_x - compass_r * 0.23, compass_y - compass_r * 0.63, strBuffer);
	...
	outtextxy(compass_x - compass_r * 0.1, compass_y - compass_r * 0.625, "H");
}
...
void draw_MAG(double compass_x, double compass_y, double side) {
	...
	outtextxy(compass_x + compass_r * 0.35, compass_y - compass_r * 0.62, "MAG");//画出MAG
}
...
void draw_PFD_attackAngle(double x, double y, double side) {
	...
	setfont(Ulength / 7 * 30, Ulength / 7 * 9, "Calibri");
	if (attackAngle >= -6 && attackAngle <= 22) {
		char num[64];
		sprintf_s(num, "%.1f", attackAngle);
		outtextxy(x - 5 * Ulength, y + Ulength * 1, num);
	}
	else if (attackAngle > 22) {
		outtextxy(x - 5 * Ulength, y + Ulength * 1, "22.0");
	}
	else if (attackAngle < -6) {
		outtextxy(x - 5 * Ulength, y + Ulength * 1, "-6.0");
	}
}
