...
char* WideCharToStringEICAS(const WCHAR* wideStr);
...
int show_eicas() {
	...
	while (is_run()) {
		...
		if (showEICASJudge == 1) {
			...
			char* mySharedMemoryEICASA = WideCharToStringEICAS(mySharedMemoryEICASW);
			...
			char* currentDirectoryEICASA  = WideCharToStringEICAS(currentDirectoryEICASW);
