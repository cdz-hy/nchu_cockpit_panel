...
//	}
	...
	double unit_side1 = getheight() / 50;
	
	//表盘初始中心
	//默认六个表盘的拟合中心是正中心
	double center_x = getwidth() / 2;
	double center_y = getheight() / 2;
	double center_x1 = getwidth() / 2 - 5 * unit_side1;
	double center_y1 = getheight() / 2 + 8 * unit_side1;
...
//			get_keymsg();
		...
		//gearFlaps为接收的数据，
		if(GetAsyncKeyState(0x62) & 0x0001){//按1襟翼升
			if(gearFlaps + Flapschange < 1){
				Flapschange+=0.125;
			}
			gearFlaps += Flapschange;
		}
		if(GetAsyncKeyState(0x61) & 0x0001){//按2襟翼降
