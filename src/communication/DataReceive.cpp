...
void data_receive(){
	...
	// Pre-allocate to prevent reallocation during flight (reallocation would crash rendering threads iterating these vectors)
	fullRoute.reserve(50000);   // ~14 hours at 1 record/sec
	routePassed.reserve(1000);
...
//				}
				...
				for(int i = 0; i < rows; i ++){
					...
					if(fabs(data[i][0] - 17) <= 0.001){
						...
						HDGMAG = rotationangle;
						ndpointrotationangle = rotationangle;
...
//			sendDREF(sock2, dref,&tmp,1);
		...
		if(nowPos < (int)route.size() && getDis(latitude, longitude, route[nowPos].lat, route[nowPos].lon) <= 3060){
