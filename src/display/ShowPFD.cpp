...
int show_pfd() {
	...
	while (is_run()) {
		...
		if (showPFDJudge == 1) {
			...
			char* mySharedMemoryPFDA = WideCharToAnsi(mySharedMemoryPFDW);
			...
			WCHAR commandLinePFDW[MAX_PATH];
			swprintf(commandLinePFDW, MAX_PATH, L"%ssubprocess\\PFD\\PFD.exe", exeDir);
			WCHAR currentDirectoryPFDW[] = L".";

			char* commandLinePFDA = WideCharToAnsi(commandLinePFDW);
			char* currentDirectoryPFDA = WideCharToAnsi(currentDirectoryPFDW);
