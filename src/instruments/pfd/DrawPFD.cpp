...
//	xyprintf(100,100,t);
	...
	// 绿色指针显示
	setcolor(EGEARGB(0x99, 79, 245, 12));
	setlinewidth(side / 200);
...
void draw_VS(double top_x, double top_y, double side) {
	double height = side / 25;
	double wide = height;
	char ARM[] = "V/S";
	setcolor(GREEN);
	setfont(height, wide, "Calibri", 0, 0, 0, 0, 0, 0);//设定字体
	...
	setfont(&FMC_font);
	ege_drawtext(ARM, top_x + side / 5.5, top_y - side / 70);
}

void draw_ALT_HOLD(double top_x, double top_y, double side) {
	double height = side / 25;
	double wide = height;
	char ARM[] = "ALT   HOLD";
	setcolor(GREEN);
...
void draw_HDG_SEL(double top_x, double top_y, double side) {
	double height = side / 25;
	double wide = height;
	char ARM[] = "HDG   SEL";
	setcolor(GREEN);
	...
	FMC_font.lfWeight = side * 3;
	setfont(&FMC_font);
	ege_drawtext(ARM, top_x, top_y - side / 70);
}

void draw_MCP_SPO(double top_x, double top_y, double side) {
	double height = side / 25;
	double wide = height;
	char ARM[] = "MCP   SPO";
	setcolor(GREEN);
...
void draw_ARM(double top_x, double top_y, double side) {
	double height = side / 25 / 1.5;
	double wide = height;
	char ARM[4] = "ARM";
	setcolor(WHITE);
	setfont(height, wide, "Calibri", 0, 0, 0, 0, 0, 0);//设定字体
	...
	FMC_font.lfWeight = side * 3;
	setfont(&FMC_font);
	ege_drawtext(ARM, top_x - side / 5.5, top_y + side / 50);
	
}

void draw_FD(double top_x, double top_y, double side) {
	double height = side / 20;
	double wide = height;
	char FD[3] = "FD";
	setcolor(GREEN);
	setfont(height, wide, "Calibri", 0, 0, 0, 0, 0, 0);//设定字体
	...
	FMC_font.lfWeight = side * 3;
	setfont(&FMC_font);
	ege_drawtext(FD,top_x,top_y + side / 6.5);
	
}

void draw_CMD(double top_x, double top_y, double side) {
	double height = side / 20;
	double wide = height;
	char FD[] = "CMD";
	setcolor(GREEN);
...
void draw_PFD_top(double top_x, double top_y, double side) {
	...
	//单开
		...
		if (altitude_mode == 1) {
			draw_ARM(top_x, top_y, side);
		}

	}
	else if(autopilot_mode == 165){
		draw_MCP_SPO(top_x, top_y, side);
		//draw_ARM(top_x, top_y, side);
	}
	else if (autopilot_mode == 162) {
		draw_HDG_SEL(top_x, top_y, side);
		draw_ARM(top_x, top_y, side);
		
	}
	else if (autopilot_mode == 16388) {
		draw_ALT_HOLD(top_x, top_y, side);
		draw_ARM(top_x, top_y, side);
		
	}else if(autopilot_mode == 52) 
	{
		draw_VS(top_x, top_y, side);
		draw_ARM(top_x, top_y, side);
	...
	if (flight_director_mode == 1) {
		draw_FD(top_x, top_y, side);
	}
	else if (flight_director_mode == 2) {
		draw_CMD(top_x, top_y, side);
	}
}
